...
exports.financialStats = async (req, res) => {
  try {
    ...
    // 4. GROUP BY CLASS (IMPORTANT FIX)
    const classMap = {};

    students.forEach(s => {
      const classId = s.classe?.id_class;
      if (!classId) return;

      if (!classMap[classId]) {
        classMap[classId] = {
          gradeFee: s.classe.grade.monthly_fee,
          studentIds: [],
        };
      }

      classMap[classId].studentIds.push(s.id_student);
    });

    // 5. PROCESS CLASS BY CLASS (Django style)
    for (const classId in classMap) {
      const { gradeFee, studentIds } = classMap[classId];
      ...
      for (const fee of classFees) {
        const isPaid = !!fee.receiptId;
        const isPassed = params.passed_months.includes(Number(fee.month));
        const isCurrent = Number(fee.month) === params.actual_month;
      ...
      // ✔ EXACT DJANGO LOGIC
      stats.total_paid_number += classPaid;
      stats.total_paid_amount += classPaid * gradeFee;

      stats.total_overdue_number += classOverdue;
      stats.total_overdue_amount += classOverdue * gradeFee;

      stats.paid_this_month_num += paidThisMonth;
      stats.paid_this_month_aut += paidThisMonth * gradeFee;

      stats.this_month_overdue_num += classSize - paidThisMonth;
      stats.this_month_overdue_aut += (classSize - paidThisMonth) * gradeFee;
    }

    // 6. IN ADVANCE (corrigé aussi)
    const inAdvanceFees = fees.filter(f =>
      params.coming_months.includes(Number(f.month)) &&
      f.receiptId
    );

    stats.in_advance_amount = inAdvanceFees.reduce((sum, f) => {
